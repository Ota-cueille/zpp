...
pub fn add(self: *program, allocator: std.mem.Allocator, child: ast.node) void {
    switch (child) {
        .binary_operator => {
            self.operations = allocator.realloc(self.operations, self.operations.len + 1) catch unreachable;
            self.operations[self.operations.len - 1] = child;
        },
